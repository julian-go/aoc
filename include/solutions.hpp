#ifndef SRC_YEAR2022_SOLUTIONS_H_
#define SRC_YEAR2022_SOLUTIONS_H_

#include <fstream>
#include <string>
#include <vector>

#include "common/problem.hpp"

std::string Day01_1(std::ifstream& in);
std::string Day01_2(std::ifstream& in);
std::string Day02_1(std::ifstream& in);
std::string Day02_2(std::ifstream& in);
std::string Day03_1(std::ifstream& in);
std::string Day03_2(std::ifstream& in);
std::string Day04_1(std::ifstream& in);
std::string Day04_2(std::ifstream& in);
std::string Day05_1(std::ifstream& in);
std::string Day05_2(std::ifstream& in);
std::string Day06_1(std::ifstream& in);
std::string Day06_2(std::ifstream& in);
std::string Day07_1(std::ifstream& in);
std::string Day07_2(std::ifstream& in);
std::string Day08_1(std::ifstream& in);
std::string Day08_2(std::ifstream& in);
std::string Day09_1(std::ifstream& in);
std::string Day09_2(std::ifstream& in);
std::string Day10_1(std::ifstream& in);
std::string Day10_2(std::ifstream& in);
std::string Day11_1(std::ifstream& in);
std::string Day11_2(std::ifstream& in);
std::string Day12_1(std::ifstream& in);
std::string Day12_2(std::ifstream& in);
std::string Day13_1(std::ifstream& in);
std::string Day13_2(std::ifstream& in);
std::string Day14_1(std::ifstream& in);
std::string Day14_2(std::ifstream& in);
std::string Day15_1(std::ifstream& in);
std::string Day15_2(std::ifstream& in);
std::string Day16_1(std::ifstream& in);
std::string Day16_2(std::ifstream& in);
std::string Day17_1(std::ifstream& in);
std::string Day17_2(std::ifstream& in);
std::string Day18_1(std::ifstream& in);
std::string Day18_2(std::ifstream& in);
std::string Day19_1(std::ifstream& in);
std::string Day19_2(std::ifstream& in);
std::string Day20_1(std::ifstream& in);
std::string Day20_2(std::ifstream& in);
std::string Day21_1(std::ifstream& in);
std::string Day21_2(std::ifstream& in);
std::string Day22_1(std::ifstream& in);
std::string Day22_2(std::ifstream& in);
std::string Day23_1(std::ifstream& in);
std::string Day23_2(std::ifstream& in);
std::string Day24_1(std::ifstream& in);
std::string Day24_2(std::ifstream& in);
std::string Day25_1(std::ifstream& in);
std::string Day25_2(std::ifstream& in);

#endif  // SRC_YEAR2022_SOLUTIONS_H_